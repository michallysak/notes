...
class UserServiceImplTest {
    ...
    @Mock
    private AuthTokenGenerator<UserValue, AuthToken> tokenGenerator;
    @InjectMocks
    ...
    void login_shouldThrow_whenUserNotFound() {
        // given
        EmailPasswordLogin login = new EmailPasswordLogin(pl.michallysak.notes.common.Email.of("notfound@example.com"), pl.michallysak.notes.auth.model.Password.of("pw"));
    ...
    void login_shouldThrow_whenNoPasswordCredential() {
        // given
        EmailPasswordLogin login = new EmailPasswordLogin(pl.michallysak.notes.common.Email.of("user@example.com"), pl.michallysak.notes.auth.model.Password.of("pw"));
    ...
    void login_shouldThrow_whenPasswordInvalid() {
        // given
        EmailPasswordLogin login = new EmailPasswordLogin(Email.of("user@example.com"), pl.michallysak.notes.auth.model.Password.of("pw"));
        User user = mock(User.class);
        PasswordCredential credential = getDummyPasswordCredential();
        when(userRepository.findByEmail(login.email())).thenReturn(Optional.of(user));
    ...
    void login_shouldMigratePassword_whenPasswordValidAndNotUpToDate() {
        // given
        EmailPasswordLogin login = new EmailPasswordLogin(Email.of("user@example.com"), Password.of("pw"));
        User user = mock(User.class);
        PasswordCredential credential = getDummyPasswordCredential();
        AuthToken token = new AuthToken("token", OffsetDateTime.now().plusHours(1));
        when(userRepository.findByEmail(login.email())).thenReturn(Optional.of(user));

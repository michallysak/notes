...
class StartupBeanTest {
    ...
    void onStart_shouldCreateAndLoginUser_andLogInfo() {
        ...
        when(userService.createUser(any(EmailPasswordCreateUser.class))).thenReturn(mock(UserValue.class));
        ...
        // then
        verify(userService).createUser(argThat(arg -> arg.email().equals(email) && arg.password().equals(password)));
        verify(userService).login(argThat(arg -> arg.email().equals(email) && arg.password().equals(password)));

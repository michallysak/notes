...
class NoteValidatorImplTestUtils {
  ...
  static Stream<CreateNote> createNotesCreationWithTitle(Stream<String> list) {
    return list.map(
        title ->
            NoteTestUtils.createCreateNoteBuilder().title(title).content("valid content").build());
  }

  static Stream<CreateNote> createNotesCreationWithContent(Stream<String> list) {
    return list.map(
        content ->
            NoteTestUtils.createCreateNoteBuilder().title("valid title").content(content).build());
  }

...
public class PasswordPolicyImpl implements PasswordPolicy {
    ...
    public boolean isUpToDate(PasswordCredential credential) {
        if (credential instanceof PBKDF2PasswordCredential pbkdf2) {
            boolean needRehash = pbkdf2.getHashedPassword().getIterations() >= pbkdf2Hasher.getIterations();
            logger.info("PBKDF2PasswordCredential with id: %s isUpToDate: %s".formatted(pbkdf2.getId(), needRehash));
            return needRehash;
        }
    ...
    public PasswordCredential hash(Password password) {
        ...
        PBKDF2HashedPassword hashedPassword = new PBKDF2HashedPassword(hash, salt, pbkdf2Hasher.getIterations());
